/*Copyright (c) 2016-2017 testing1.com All Rights Reserved.
 This software is the confidential and proprietary information of testing1.com You shall not disclose such Confidential Information and shall use it only in accordance
 with the terms of the source code license agreement you entered into with testing1.com*/
package com.test_21.qadatabase.controller;

/*This is a Studio Managed File. DO NOT EDIT THIS FILE. Your changes may be reverted by Studio.*/
import com.test_21.qadatabase.service.Table20Service;
import java.io.*;
import javax.servlet.http.HttpServletRequest;
import javax.servlet.http.HttpServletResponse;
import org.slf4j.Logger;
import org.slf4j.LoggerFactory;
import org.hibernate.TypeMismatchException;
import org.springframework.data.domain.Page;
import org.springframework.data.domain.Pageable;
import org.springframework.web.bind.annotation.PathVariable;
import org.springframework.web.bind.annotation.RestController;
import org.springframework.web.bind.annotation.RequestBody;
import org.springframework.web.bind.annotation.RequestMapping;
import org.springframework.web.bind.annotation.RequestMethod;
import org.springframework.web.bind.annotation.RequestParam;
import org.springframework.web.multipart.MultipartHttpServletRequest;
import org.springframework.beans.factory.annotation.Autowired;
import org.springframework.beans.factory.annotation.Qualifier;
import com.wavemaker.runtime.data.exception.EntityNotFoundException;
import com.wavemaker.runtime.data.expression.QueryFilter;
import com.wavemaker.runtime.util.WMMultipartUtils;
import com.wavemaker.runtime.util.WMRuntimeUtils;
import com.wavemaker.runtime.file.model.DownloadResponse;
import com.wordnik.swagger.annotations.*;
import com.test_21.qadatabase.*;
import com.test_21.qadatabase.service.*;
import com.wavemaker.tools.api.core.annotations.WMAccessVisibility;
import com.wavemaker.tools.api.core.models.AccessSpecifier;

/**
 * Controller object for domain model class Table20.
 * @see com.test_21.qadatabase.Table20
 */
@RestController(value = "Qadatabase.Table20Controller")
@RequestMapping("/qadatabase/Table20")
@Api(description = "Exposes APIs to work with Table20 resource.", value = "Table20Controller")
public class Table20Controller {

    private static final Logger LOGGER = LoggerFactory.getLogger(Table20Controller.class);

    @Autowired
    @Qualifier("qadatabase.Table20Service")
    private Table20Service table20Service;

    @RequestMapping(value = "/search", method = RequestMethod.POST)
    @ApiOperation(value = "Returns the list of Table20 instances matching the search criteria.")
    public Page<Table20> findTable20s(Pageable pageable, @RequestBody QueryFilter[] queryFilters) {
        LOGGER.debug("Rendering Table20s list");
        return table20Service.findAll(queryFilters, pageable);
    }

    @RequestMapping(value = "/", method = RequestMethod.GET)
    @ApiOperation(value = "Returns the list of Table20 instances.")
    public Page<Table20> getTable20s(Pageable pageable) {
        LOGGER.debug("Rendering Table20s list");
        return table20Service.findAll(pageable);
    }

    /**
	 * This setter method should only be used by unit tests
	 * 
	 * @param service
	 */
    protected void setTable20Service(Table20Service service) {
        this.table20Service = service;
    }

    @RequestMapping(value = "/", method = RequestMethod.POST)
    @WMAccessVisibility(value = AccessSpecifier.APP_ONLY)
    @ApiOperation(value = "Creates a new Table20 instance.")
    public Table20 createTable20(@RequestBody Table20 instance) {
        LOGGER.debug("Create Table20 with information: {}", instance);
        instance = table20Service.create(instance);
        LOGGER.debug("Created Table20 with information: {}", instance);
        return instance;
    }

    @RequestMapping(value = "/count", method = RequestMethod.GET)
    @WMAccessVisibility(value = AccessSpecifier.APP_ONLY)
    @ApiOperation(value = "Returns the total count of Table20 instances.")
    public Long countAllTable20s() {
        LOGGER.debug("counting Table20s");
        Long count = table20Service.countAll();
        return count;
    }

    @RequestMapping(value = "/{id}", method = RequestMethod.GET)
    @WMAccessVisibility(value = AccessSpecifier.APP_ONLY)
    @ApiOperation(value = "Returns the Table20 instance associated with the given id.")
    public Table20 getTable20(@PathVariable(value = "id") Integer id) throws EntityNotFoundException {
        LOGGER.debug("Getting Table20 with id: {}", id);
        Table20 instance = table20Service.findById(id);
        LOGGER.debug("Table20 details with id: {}", instance);
        return instance;
    }

    @RequestMapping(value = "/{id}", method = RequestMethod.PUT)
    @WMAccessVisibility(value = AccessSpecifier.APP_ONLY)
    @ApiOperation(value = "Updates the Table20 instance associated with the given id.")
    public Table20 editTable20(@PathVariable(value = "id") Integer id, @RequestBody Table20 instance) throws EntityNotFoundException {
        LOGGER.debug("Editing Table20 with id: {}", instance.getId());
        instance.setId(id);
        instance = table20Service.update(instance);
        LOGGER.debug("Table20 details with id: {}", instance);
        return instance;
    }

    @RequestMapping(value = "/{id}", method = RequestMethod.DELETE)
    @WMAccessVisibility(value = AccessSpecifier.APP_ONLY)
    @ApiOperation(value = "Deletes the Table20 instance associated with the given id.")
    public boolean deleteTable20(@PathVariable(value = "id") Integer id) throws EntityNotFoundException {
        LOGGER.debug("Deleting Table20 with id: {}", id);
        Table20 deleted = table20Service.delete(id);
        return deleted != null;
    }
}
